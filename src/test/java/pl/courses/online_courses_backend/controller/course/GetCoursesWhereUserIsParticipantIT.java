...
class GetCoursesWhereUserIsParticipantIT extends BaseTest {

    private final String PATH = "/api/courses/get-courses-where-user-is-participant";

    @DisplayName("Should return one course")
    @Test
    void shouldReturnOneCourse() throws Exception {
    ...
    @DisplayName("Should no return course")
    @Test
    void shouldNoReturnCourse() throws Exception {
        ...
        response.andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.size()").value(0L));
    }

    @DisplayName("Should throw exception cause no user")
    @Test
    void shouldThrowExceptionCauseNoUser() throws Exception {
